...
export function DataEntry() {
    ...
    return state.transactions
      .filter((t) => {
        ...
        if (searchQuery) {
          ...
          const valCommentMatch =
            t.validationComment?.toLowerCase().includes(q) || false;
          if (
            !fullName.includes(q) &&
            !phoneMatch &&
            !commentsMatch &&
  ...
  const transactionLanes = useMemo(() => {
    const lanes: Record<string, number> = {};
    const txs = [...state.transactions].sort((a, b) => {
      const startA = parseUTCDate(a.date).getTime();
      const startB = parseUTCDate(b.date).getTime();
      if (startA !== startB) return startA - startB;
      // If same start date, sort by nights (longer first to occupy lower lanes)
      return (b.nights || 1) - (a.nights || 1);
    });

    const laneEnds: number[] = [];

    txs.forEach((t) => {
      const start = parseUTCDate(t.date).getTime();
  ...
  React.useEffect(() => {
    setPlatformData((prev) => {
      const next: Record<
        string,
        { checked: boolean; amount: string; clientAmount: string }
      > = {};
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Informations Locataire (Optionnel)
                </label>
                                ...
                                onChange={(e) => {
                                  ...
                                  setPlatformData((prev) => {
                                    ...
                                    if (pData.clientAmount) {
                                      const c =
                                        parseFloat(pData.clientAmount) || 0;
                                      const c_comm = parseFloat(val) || 0;
                                      const c_bank =
                                        parseFloat(pData.bankFee) || 0;
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            {showSearch && (
              <div
                className="p-4 bg-blue-50/20 border-b border-gray-200 space-y-4 animate-fade-in"
                id="client-search-filters"
              >
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-sm text-gray-500">
                            ...
                            {t.comments && (
                              <div
                                className="text-xs text-gray-400 mt-1 italic whitespace-normal max-w-[220px]"
                            ...
                            <div className="flex flex-col">
                              <span>
                                ...
                                <span className="text-gray-500 text-xs font-normal ml-1">
                                  ({t.nights || 1}{" "}
                                  {t.nights && t.nights > 1 ? "nuits" : "nuit"})
                                </span>
                              </span>
                              {t.clientAmount && (
                                <span className="text-xs text-gray-500 font-normal">
                                  Client: {t.clientAmount} €
                                </span>
                              )}
                            </div>
                          </td>
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100 sticky top-20">
            ...
            <div className="grid grid-cols-7 gap-1">
              {calendarDays.map((date, idx) => {
                ...
                const dayTxs = state.transactions
                  .filter((t) => {
                ...
                return (
                  <div
                    key={dateStr}
                    className={`relative p-1 min-h-[64px] text-sm text-center rounded-md cursor-pointer hover:bg-gray-100 flex flex-col items-center justify-start
                      ${dateStr === new Date().toISOString().split("T")[0] ? "ring-1 ring-blue-400 font-bold text-blue-600" : ""}
                    `}
                    onClick={() => setDate(dateStr)}
                  >
                    <span className="z-10 relative inline-flex items-center justify-center w-6 h-6 rounded-full bg-white/90 font-medium shadow-sm backdrop-blur-sm">
                      {date.getDate()}
                    </span>

                    {/* Platform bars */}
                    <div className="absolute top-8 left-0 right-0 flex flex-col gap-[1px] z-0">
                      {dayTxs.length > 0 &&
                        Array.from({
                          length:
                            Math.max(
                              ...dayTxs.map((t) => transactionLanes[t.id] || 0),
                              -1,
                            ) + 1,
                        }).map((_, i) => {
                          const t = dayTxs.find(
                            (tx) => (transactionLanes[tx.id] || 0) === i,
                          );

                          if (!t)
                            return (
                              <div key={`empty-${i}`} className="h-[14px]" />
                            );

                          const isStart = t.date === dateStr;
                          const endD = parseUTCDate(t.date);
                          endD.setUTCDate(
                            endD.getUTCDate() + (t.nights || 1) - 1,
                          );
                          ...
                          return (
                            <div
                              key={t.id}
                              className="h-[14px] cursor-pointer group relative w-full flex items-center"
                              onClick={(e) => {
                                e.stopPropagation();
                                setEditingId(t.id);
                                setEditData(t);
                            ...
                            >
                              <div
                                className={`h-2.5 absolute ${isStart ? "rounded-l-md" : ""} ${isEnd ? "rounded-r-md" : ""} flex items-center transition-opacity group-hover:opacity-80 shadow-sm`}
                                style={{
                                  backgroundColor:
                                    state.settings.platformColors?.[
                                      t.platform
                                    ] || "#9ca3af",
                                  left: isStart ? "2px" : "-2.5px",
                                  right: isEnd ? "2px" : "-2.5px",
                                }}
                              >
                                {isStart && (
                                  <div
                                    className={`w-1.5 h-1.5 ${t.isValidated ? "bg-emerald-500 border-emerald-600" : "bg-white border-gray-400"} border rounded-full absolute left-0.5 shadow-sm`}
                                  />
                                )}
                              </div>
                            </div>
                ...
                {state.settings.platforms.map((p) => (
                  ...
                  >
                    <div
                      ...
                      style={{
                        backgroundColor:
                          state.settings.platformColors?.[p] || "#ccc",
                      }}
                    />
                    {p}
                  </div>
