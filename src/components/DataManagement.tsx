...
export function DataManagement() {
  const { state, clearDatabase, importTransactionsSmart, updateSettings } =
    useStore();
  ...
  const handleExport = () => {
    try {
      ...
      downloadAnchor.setAttribute(
        "download",
        `locatrack_export_${dateStr}.json`,
      );
  ...
  const handleClearDatabase = async () => {
    ...
    try {
      ...
      setStatus({
        type: "success",
        message:
          "La base de données a été vidée avec succès. Toutes vos réservations ont été supprimées.",
      });
      setImportResult(null);
    ...
    <div className="space-y-8 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Gérez la sauvegarde, la restauration, la détection des doublons et la
          réinitialisation de votre base de données LocaTrack.
        </p>
      </div>
      ...
      {status && (
        ...
        >
          <CheckCircle
            className={`w-5 h-5 shrink-0 ${status.type === "success" ? "text-green-600" : "text-red-600"}`}
          />
          <div>
            ...
            {status.type === "success" && importResult && (
              <div className="mt-2 text-sm text-green-700 space-y-1">
                <p>
                  • <strong>{importResult.imported}</strong> nouvelles
                  réservations importées.
                </p>
                <p>
                  • <strong>{importResult.updated}</strong> réservations
                  existantes mises à jour.
                </p>
                <p>
                  • <strong>{importResult.skipped}</strong> doublons ignorés.
                </p>
                {importResult.settingsImported && (
                  <p>
                    • Configuration de l'application (plateformes, taxes et
                    coûts) importée.
                  </p>
                )}
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 flex flex-col justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mb-4 leading-relaxed">
              Téléchargez une sauvegarde complète contenant l'intégralité de vos
              réservations, ainsi que vos réglages personnalisés (plateformes de
              réservation, codes couleurs associés, investissements immobiliers
              et grilles de taxes annuelles).
            </p>
            <div className="bg-gray-50 border border-gray-100 rounded-md p-3 mb-6">
              ...
              <div className="flex justify-between text-xs text-gray-500 mb-1">
                ...
                <span className="font-semibold text-gray-800">
                  {state.settings.platforms.length}
                </span>
              </div>
              <div className="flex justify-between text-xs text-gray-500">
                <span>Coûts d'investissement:</span>
                <span className="font-semibold text-gray-800">
                  {state.settings.houseCosts.length}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          ...
          <p className="text-sm text-gray-600 mb-4 leading-relaxed">
            Glissez-déposez ou sélectionnez un fichier de sauvegarde au format{" "}
            <code>.json</code> pour restaurer vos réservations et réglages.
          </p>
          ...
          >
            ...
            />
            <Upload
              className={`w-8 h-8 mx-auto mb-2 ${importFile ? "text-emerald-500" : "text-gray-400"}`}
            />
            {importFile ? (
              <div>
            ...
            <div className="mt-4 p-4 border border-gray-200 rounded-md bg-gray-50 space-y-4">
              <div className="text-xs text-gray-600 space-y-1">
                <p className="font-semibold text-gray-800">
                  Contenu détecté dans le fichier :
                </p>
                <p>
                  • <strong>{parsedData.transactions?.length || 0}</strong>{" "}
                  réservations / lignes de revenus.
                </p>
                {parsedData.settings && (
                  <p>
                    •{" "}
                    <strong>
                      {parsedData.settings.platforms?.length || 0}
                    </strong>{" "}
                    plateformes et autres configurations de réglages.
                  </p>
                )}
              </div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1 flex items-center gap-1">
                    ...
                    <HelpCircle
                      ...
                      title="Un doublon est détecté sur le même ID unique, ou sur la même date avec une plateforme et un montant identiques."
                    />
                  </label>
                  <select
                    value={strategy}
                  ...
                  >
                    ...
                    <option value="all">
                      Tout importer (Générer des copies pour tous)
                    </option>
                  </select>
                </div>
                ...
                {parsedData.settings && (
                  <label className="flex items-center space-x-2 cursor-pointer pt-1">
                    ...
                    <span className="text-xs text-gray-700 font-medium">
                      Importer également la configuration de l'application
                    </span>
                  </label>
                )}
              </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        <h3 className="text-lg font-medium text-gray-900 mb-4 flex items-center gap-2 border-b pb-2">
          <Save className="w-5 h-5 text-indigo-500" />
          Sauvegarde Automatique
        </h3>

        <div className="bg-indigo-50 text-indigo-800 p-4 rounded-md text-sm mb-6 flex items-start gap-3">
          <HelpCircle className="w-5 h-5 mt-0.5 shrink-0" />
          <div>
            <p className="font-semibold mb-1">Comment ça marche ?</p>
            <p>
              La sauvegarde automatique génère un fichier <code>.json</code> qui
              se télécharge directement sur votre ordinateur ou appareil mobile.
              <strong>
                {" "}
                Aucune connexion à une base de données ou à un cloud n'est
                nécessaire.
              </strong>{" "}
              L'application vérifie la date de votre dernière sauvegarde à
              chaque ouverture, et si le délai est dépassé (ex: 1 semaine), elle
              vous proposera le téléchargement du fichier.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center">
          ...
          {state.settings.autoBackup && (
            <div className="flex items-center space-x-2 ml-4">
              ...
              <select
                value={state.settings.autoBackupInterval || "weekly"}
                onChange={(e) =>
                  updateSettings({
                    autoBackupInterval: e.target.value as "weekly" | "monthly",
                  })
                }
        ...
        {state.settings.autoBackup && (
          <div className="mt-6 border-t border-gray-200 pt-4">
            ...
            {state.settings.backupHistory &&
            state.settings.backupHistory.length > 0 ? (
              <ul className="space-y-2">
                {state.settings.backupHistory.map((dateStr, idx) => (
                  <li
                    key={idx}
                    className="text-sm text-gray-600 flex items-center gap-2"
                  >
                    <CheckCircle className="w-4 h-4 text-emerald-500" />
                    Sauvegarde effectuée le{" "}
                    {new Date(dateStr).toLocaleDateString("fr-FR")} à{" "}
              ...
              <p className="text-sm text-gray-500 italic">
                Aucune sauvegarde automatique n'a encore été effectuée.
              </p>
            )}
          </div>
        )}
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-red-200 bg-red-50/20">
        ...
        <p className="text-sm text-red-800 leading-relaxed mb-4">
          ...
          l'intégralité de vos réservations et revenus enregistrés et
          réinitialisera la configuration globale (plateformes, grilles d'impôts
          et investissements de maison) à leurs valeurs d'origine. Veuillez
          exporter vos données avant de procéder si vous souhaitez conserver une
          sauvegarde.
        </p>
