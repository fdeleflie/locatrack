...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const interval = state.settings.autoBackupInterval === "monthly" ? 30 : 7;
    const now = new Date();
    const lastBackup = state.settings.lastBackupDate
      ? new Date(state.settings.lastBackupDate)
      : new Date(0);
    ...
    if (diffDays >= interval) {
      try {
        ...
        downloadAnchor.setAttribute(
          "download",
          `locatrack_autobackup_${dateStr}.json`,
        );
    ...
    state.settings.autoBackupInterval,
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <nav className="flex items-center space-x-1 sm:space-x-4">
              ...
              <button
                ...
                title="Déconnexion"
              >
                <span className="hidden sm:inline mr-2 text-xs truncate max-w-[120px] lg:max-w-none">
