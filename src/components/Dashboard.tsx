...
export function Dashboard() {
  ...
  const years = useMemo(() => {
    const y = new Set<string>(
      state.transactions.map((t) => t.date.substring(0, 4)),
    );
  ...
  const distributedDays = useMemo(() => {
    ...
    state.transactions.forEach((t) => {
      ...
      let fiscalAmount =
        t.clientAmount !== undefined &&
        t.clientAmount !== null &&
        !isNaN(t.clientAmount) &&
        t.clientAmount > 0
          ? t.clientAmount
          : t.amount;
  ...
  const yearData = useMemo(() => {
    const yearDays = distributedDays.filter((d) =>
      d.date.startsWith(selectedYear),
    );
    const previousYearStr = (parseInt(selectedYear) - 1).toString();
    const previousYearDays = distributedDays.filter((d) =>
      d.date.startsWith(previousYearStr),
    );

    const nights = yearDays.length;
    const gross = yearDays.reduce((acc, curr) => acc + curr.amount, 0);
    const fiscalGross = yearDays.reduce(
      (acc, curr) => acc + curr.fiscalAmount,
      0,
    );

    const previousGross = previousYearDays.reduce(
    ...
    const currentYearTaxes = state.settings.yearlyTaxes?.[selectedYear] ||
      state.settings.yearlyTaxes?.[new Date().getFullYear().toString()] || {
        csgRate: 18.6,
        taxRate: 11,
        abattementRate: 70,
        chargeParNuit: 5,
    ...
    const prevYearTaxes =
      state.settings.yearlyTaxes?.[(parseInt(selectedYear) - 1).toString()] ||
      currentYearTaxes;
    ...
    const blackGross = yearDays
      .filter(
        (d) =>
          d.platform === "Black" ||
          d.platform.toLowerCase() === "black" ||
          state.settings.platformExcludeFiscal?.[d.platform],
      )
    ...
    const estimatedTaxes =
      declaredGross *
      (currentYearTaxes.abattementRate / 100) *
      (currentYearTaxes.csgRate / 100);
    ...
    // Previous year net
    const prevBlackGross = previousYearDays
      .filter(
        (d) =>
          d.platform === "Black" ||
          d.platform.toLowerCase() === "black" ||
    ...
    const prevEstimatedTaxes =
      prevDeclaredGross *
      (prevYearTaxes.abattementRate / 100) *
      (prevYearTaxes.csgRate / 100);
  ...
  const yearDataToDate = useMemo(() => {
    ...
    const yearDays = distributedDays.filter(
      (d) => d.date.startsWith(selectedYear) && d.date <= today,
    );
    const previousYearStr = (parseInt(selectedYear) - 1).toString();
    const prevToday = new Date(
      ...
      .toISOString()
      .split("T")[0];
    const previousYearDays = distributedDays.filter(
      (d) => d.date.startsWith(previousYearStr) && d.date <= prevToday,
    );
  ...
  const monthlyData = useMemo(() => {
    return months.map((monthName, index) => {
      ...
      const yTaxes = state.settings.yearlyTaxes?.[selectedYear] ||
        state.settings.yearlyTaxes?.[new Date().getFullYear().toString()] || {
          csgRate: 18.6,
          taxRate: 11,
          abattementRate: 70,
      ...
      const estimatedTaxes =
        declaredGross * (yTaxes.abattementRate / 100) * (yTaxes.csgRate / 100);
      const net = gross - estimatedTaxes - variableCharges - fixedCharges;
  ...
  const yearlyChartData = useMemo(() => {
    return years
      .map((y) => {
        ...
        const yTaxes = state.settings.yearlyTaxes?.[y] ||
          state.settings.yearlyTaxes?.[new Date().getFullYear().toString()] || {
            csgRate: 18.6,
            taxRate: 11,
            abattementRate: 70,
  ...
  const monthlyComparisonData = useMemo(() => {
    return months.map((monthName, index) => {
      ...
      years.forEach((y) => {
        ...
        const days = distributedDays.filter((d) =>
          d.date.startsWith(monthPrefix),
        );
        const nights = days.length;
        const gross = days.reduce((acc, curr) => acc + curr.amount, 0);
  ...
  const totalInvestment = useMemo(() => {
    return (
      state.settings.houseCosts?.reduce((acc, curr) => acc + curr.amount, 0) ||
      0
    );
  ...
  const expensesData = useMemo(() => {
    return [
      {
        name: "Impôts estimés",
        value: Math.round(yearData.breakdown.estimatedTaxes),
        color: "#ef4444",
      },
      {
        name: "Charges variables",
        value: Math.round(yearData.breakdown.variableCharges),
        color: "#f59e0b",
      },
      {
        name: "Charges fixes",
        value: Math.round(yearData.breakdown.fixedCharges),
        color: "#3b82f6",
      },
      { name: "Revenu Net", value: Math.round(yearData.net), color: "#10b981" },
    ].filter((d) => d.value > 0);
  }, [yearData]);

  const platformData = useMemo(() => {
    return Object.keys(yearData.platformStats)
      .map((platform, index) => {
        const defaultColors = [
          "#2563eb",
          "#16a34a",
        ...
        return {
          name: platform,
          value: Math.round(yearData.platformStats[platform].gross),
          nights: yearData.platformStats[platform].nights,
          color:
            state.settings.platformColors?.[platform] ||
            defaultColors[index % defaultColors.length],
        };
      })
      .filter((d) => d.value > 0);
  }, [yearData.platformStats, state.settings.platformColors]);
  ...
  const isPastYear = parseInt(selectedYear) < parseInt(currentYearStr);
  const yearStatusLabel = isPastYear ? "(Réalisé)" : "(Projections incluses)";
  ...
  const comparisonData = useMemo(() => {
    ...
    return years.map((y) => {
      const periodDays = distributedDays.filter(
        (d) => d.date.startsWith(y) && isBeforeOrOnTodayMD(d.date),
      );
      const gross = periodDays.reduce((acc, curr) => acc + curr.amount, 0);
      const nights = periodDays.length;
      const blackGross = periodDays
        .filter(
          (d) =>
            d.platform === "Black" ||
            d.platform.toLowerCase() === "black" ||
      ...
      const yTaxes = state.settings.yearlyTaxes?.[y] ||
        state.settings.yearlyTaxes?.[currentYearStr] || {
          csgRate: 18.6,
          taxRate: 11,
          abattementRate: 70,
          chargeParNuit: 5,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 lg:col-span-1">
          ...
          <p className="mt-2 text-2xl font-bold text-gray-900">
            {Math.round(yearData.gross).toLocaleString()} €
          </p>
        </div>
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 lg:col-span-1">
          <p className="text-sm font-medium text-gray-500">Revenus Net</p>
          <p className="mt-2 text-2xl font-bold text-green-600">
            {Math.round(yearData.net).toLocaleString()} €
          </p>
        </div>
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 lg:col-span-1">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 lg:col-span-1">
          ...
          <p className="mt-2 text-2xl font-bold text-emerald-700">
            {totalNetAllYears.toLocaleString()} €
          </p>
        </div>
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 lg:col-span-1">
          <p className="text-sm font-medium text-gray-500">Coût Maison</p>
          <p className="mt-2 text-2xl font-bold text-amber-600">
            {remainingInvestment > 0 ? remainingInvestment.toLocaleString() : 0}{" "}
            €
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-blue-100 p-6">
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-6 border-b border-gray-100 pb-4">
          <div>
            <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
              ...
              Comparateur d'avancement à date (du 1er Janvier au{" "}
              {formattedToday})
            </h3>
            <p className="text-xs text-gray-500 mt-1">
              Comparez vos performances cumulées de chaque année sur la même
              période pour savoir si vous êtes en avance ou en retard.
            </p>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50 font-semibold">
                <tr>
                  ...
                  <th
                    scope="col"
                    className="px-4 py-2 text-right text-[11px] font-bold text-gray-500 uppercase tracking-wider"
                  >
                    Nuits cumulées
                  </th>
                  <th
                  ...
                  <th
                    scope="col"
                    className="px-4 py-2 text-center text-[11px] font-bold text-gray-500 uppercase tracking-wider"
                  >
                    Écart vs {comparisonRefYear}
                  </th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {comparisonData.map((d) => {
                  ...
                  return (
                    <tr
                      key={d.year}
                      className={`${isRef ? "bg-blue-50/50 font-semibold" : "hover:bg-gray-50/50"}`}
                    >
                      ...
                      <td className="px-4 py-2.5 whitespace-nowrap text-sm text-gray-700 text-right">
                        {d.nights} nuits
                      </td>
                      <td className="px-4 py-2.5 whitespace-nowrap text-sm text-gray-700 text-right">
                        {d.occupancyRate}%
                      ...
                      <td
                        className={`px-4 py-2.5 whitespace-nowrap text-sm text-right font-bold ${isRef ? "text-blue-700" : "text-gray-900"}`}
                      >
                      ...
                      <td className="px-4 py-2.5 whitespace-nowrap text-xs text-center">
                        {isRef ? (
                          ...
                          <div className="flex flex-col items-center justify-center gap-0.5">
                            ...
                            <span
                              className={`text-[10px] font-bold ${isAhead ? "text-green-600" : "text-red-600"}`}
                            >
                              {isAhead ? "En avance de +" : "En retard de "}
                              {pct.toFixed(1)}%
                            </span>
                          </div>
                        )}
        ...
        <div className="mt-6 pt-5 border-t border-gray-100">
          <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-4">
            Visualisation graphique : Revenu Net cumulé au {formattedToday} par
            année
          </h4>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <RechartsTooltip
                  formatter={(value) => [`${Math.round(Number(value))} €`, "Net cumulé à date"]}
                  cursor={{ fill: "#f3f4f6" }}
                />
                <Bar dataKey="net" radius={[4, 4, 0, 0]} maxBarSize={50}>
      ...
      {/* Dynamic Selected Year Insights */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 lg:col-span-2">
          <h3 className="text-lg font-medium text-gray-900 mb-4">
            Évolution mensuelle ({selectedYear}) <span className="text-sm font-normal text-gray-500 ml-2">{yearStatusLabel}</span>
          </h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <h3 className="text-lg font-medium text-gray-900 mb-4">
            Plateformes ({selectedYear}) <span className="text-sm font-normal text-gray-500 ml-2">{yearStatusLabel}</span>
          </h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <RechartsTooltip
                  formatter={(value) => [`${Math.round(Number(value))} €`, "Brut"]}
                />
              </PieChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 lg:col-span-3">
          <h3 className="text-lg font-medium text-gray-900 mb-4">
            Structure des revenus et charges ({selectedYear}) <span className="text-sm font-normal text-gray-500 ml-2">{yearStatusLabel}</span>
          </h3>
          <div className="h-40 flex flex-col items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <Bar
                  dataKey="fixedCharges"
                  name="Charges fixes"
                  stackId="a"
                  fill="#3b82f6"
                />
                <Bar
                  dataKey="variableCharges"
                  name="Charges variables"
                  stackId="a"
                  fill="#f59e0b"
                />
                <Bar
                  dataKey="estimatedTaxes"
                  name="Impôts"
                  stackId="a"
                  fill="#ef4444"
                  radius={[0, 4, 4, 0]}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <h3 className="text-lg font-medium text-gray-900 mb-4">
            Revenu Net par Année <span className="text-sm font-normal text-gray-500 ml-2">(Réalisé & Projections)</span>
          </h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
              ...
              >
                ...
                />
                <RechartsTooltip
                  formatter={(value) => [`${Math.round(Number(value))} €`, "Net"]}
                  cursor={{ fill: "#f3f4f6" }}
                />
                <Bar
                  dataKey="net"
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <h3 className="text-lg font-medium text-gray-900 mb-4">
            Comparatif Mensuel (Net) <span className="text-sm font-normal text-gray-500 ml-2">(Réalisé & Projections)</span>
          </h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
              ...
              >
                ...
                />
                <RechartsTooltip
                  formatter={(value, name) => [`${Math.round(Number(value))} €`, `Année ${name}`]}
                />
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <RechartsTooltip
                  formatter={(value) => [`${Math.round(Number(value))} %`, "Taux d'occupation"]}
                  cursor={{ fill: "#f3f4f6" }}
                />
                <Bar
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <h3 className="text-lg font-medium text-gray-900 mb-4">
            Prix Moyen Net par Nuit <span className="text-sm font-normal text-gray-500 ml-2">(Réalisé & Projections)</span>
          </h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
              ...
              >
                ...
                />
                <RechartsTooltip
                  formatter={(value) => [`${Math.round(Number(value))} €`, "Prix moyen net"]}
                  cursor={{ fill: "#f3f4f6" }}
                />
                <Bar
                  dataKey="netPerNight"
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="px-6 py-3 text-center text-sm font-semibold text-gray-900 w-1/3">
                  Projeté
                </th>
                <th className="px-6 py-3 text-center text-sm font-semibold text-gray-900 w-1/3">
                  À date
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              <tr>
                ...
                <td className="px-6 py-3 text-sm text-center text-gray-900">
                  {yearData.occupancyRate.toFixed(1)} %
                </td>
                <td className="px-6 py-3 text-sm text-center text-gray-900 bg-gray-50/50">
                  {yearDataToDate.occupancyRate.toFixed(1)} %
                </td>
              </tr>
              {/* Platform breakdown */}
              <tr>
                <td className="px-6 py-3 text-sm font-medium text-gray-900">
                  <div className="flex flex-col gap-1">
                    <span className="font-semibold mb-1 border-b border-gray-200 pb-1">
                      Plateformes
                    </span>
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="flex gap-2 font-medium border-b border-gray-200 pb-1 text-gray-700 justify-center">
                      {state.settings.platforms.map((p) => (
                        <div
                          ...
                          style={{ color: state.settings.platformColors?.[p] }}
                        >
                          {p.substring(0, 3)}
                        </div>
                      ))}
                    ...
                    <div className="flex gap-2 justify-center">
                      {state.settings.platforms.map((p) => (
                        ...
                        >
                          {yearData.platformStats[p]?.nights || 0}
                        </div>
                      ))}
                    </div>
                    <div className="flex gap-2 justify-center">
                      {state.settings.platforms.map((p) => (
                        <div
                          ...
                          className="flex-1 text-center text-gray-900"
                        >
                          {Math.round(yearData.platformStats[p]?.gross || 0)}
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="flex gap-2 justify-center">
                      {state.settings.platforms.map((p) => (
                        <div
                          ...
                          className="flex-1 text-center text-gray-900"
                        >
                          {yearDataToDate.platformStats[p]?.nights || 0}
                        </div>
                      ))}
                    </div>
                    <div className="flex gap-2 justify-center">
                      {state.settings.platforms.map((p) => (
                        ...
                        >
                          {Math.round(
                            yearDataToDate.platformStats[p]?.gross || 0,
                          )}
                        </div>
                      ))}
                    </div>
              ...
              <tr>
                ...
                <td className="px-6 py-3 text-sm font-bold text-gray-900 text-center bg-gray-50/50">
                  {yearDataToDate.nights}
                </td>
              </tr>
              <tr>
                <td className="px-6 py-3 text-sm font-medium text-gray-900">
                ...
                <td className="px-6 py-3 text-sm font-bold text-gray-900 text-center">
                  {Math.round(yearData.fiscalGross)}
                </td>
                <td className="px-6 py-3 text-sm font-bold text-gray-900 text-center bg-gray-50/50">
                  {Math.round(yearDataToDate.fiscalGross)}
                </td>
              </tr>
              <tr>
                <td className="px-6 py-3 text-sm font-medium text-gray-900 bg-gray-50">
                ...
                <td className="px-6 py-3 text-sm font-bold text-gray-900 text-center">
                  {Math.round(yearData.gross)}
                </td>
                <td className="px-6 py-3 text-sm font-bold text-gray-900 text-center bg-gray-50/50">
                  {Math.round(yearDataToDate.gross)}
                </td>
              </tr>
              <tr>
                <td className="px-6 py-3 text-sm font-medium text-gray-900">
                ...
                <td className="px-6 py-3 text-sm font-bold text-green-700 text-center">
                  {Math.round(yearData.net)}
                </td>
                <td className="px-6 py-3 text-sm font-bold text-green-700 text-center bg-gray-50/50">
                  {Math.round(yearDataToDate.net)}
                </td>
              </tr>
              {/* Monthly averages */}
              <tr>
                ...
                <td className="px-6 py-3 text-sm text-gray-900 text-center">
                  {Math.round(yearData.gross / 12)}
                </td>
                <td className="px-6 py-3 text-sm text-gray-400 text-center bg-gray-50/50">
                  -
                </td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td className="px-6 py-3 text-sm text-gray-900 text-center">
                  {yearData.nights > 0
                    ? Math.round(yearData.gross / yearData.nights)
                    : 0}
                </td>
                <td className="px-6 py-3 text-sm text-gray-900 text-center bg-gray-50/50">
                  {yearDataToDate.nights > 0
                    ? Math.round(yearDataToDate.gross / yearDataToDate.nights)
                    : 0}
                </td>
              </tr>
              <tr>
                ...
                <td className="px-6 py-3 text-sm text-gray-900 text-center">
                  {yearData.nights > 0
                    ? Math.round(yearData.net / yearData.nights)
                    : 0}
                </td>
                <td className="px-6 py-3 text-sm text-gray-900 text-center bg-gray-50/50">
                  {yearDataToDate.nights > 0
                    ? Math.round(yearDataToDate.net / yearDataToDate.nights)
                    : 0}
                </td>
              </tr>
            </tbody>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th
                  scope="col"
                  className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-1/3"
                >
                  Brut
                </th>
                <th
